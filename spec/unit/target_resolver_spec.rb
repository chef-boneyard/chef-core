...
RSpec.describe ChefCore::TargetResolver do
  let(:target_string) { "" }
  let(:default_protocol) { "ssh" }
  let(:connection_options) { {} }
  subject { ChefCore::TargetResolver.new(target_string, default_protocol, connection_options) }
  ...
  context "#expand_targets" do
    xit "prefers the config option max_expanded_targets over MAX_EXPANDED_TARGETS", "Please write this test" do
    end
    ...
    it "raises InvalidRange if a range mixes alpha and numeric" do
      expect { subject.expand_targets("host[a:9]") }.to raise_error(ChefCore::TargetResolver::InvalidRange)
    end

    it "raises TooManyRanges if more than two ranges are included" do
      expect { subject.expand_targets("[0:1][5:10][10:11]") }.to raise_error(ChefCore::TargetResolver::TooManyRanges)
    end

    context "when the target resolves to more than #{ChefCore::TargetResolver::MAX_EXPANDED_TARGETS} names" do
      it "raises TooManyTargets" do
        expect { subject.expand_targets("[0:99999]") }.to raise_error(ChefCore::TargetResolver::TooManyTargets)
      end
    end
  end
  ...
  context "#make_credentials" do
    ...
    subject do
      ...
      resolver = ChefCore::TargetResolver.new("", default_protocol, opts)
      ...
      context "and it is not valid" do
        it "raises an error" do
          expect { subject.prefix_from_target("bad://host.com") }
            .to raise_error(ChefCore::TargetResolver::UnsupportedProtocol)
        end
      end
    end
  end
